...
var x = d3.scaleLinear().range([0, w]); // initial -- hardcoded data
...
function loadInitialData() {
  ...
  barDataset = [[parseInt("4323160"), '10'], [parseInt("4275439"), '9'], [parseInt("3947420"), '8'], [parseInt("3307383"), '7'], [parseInt("3188386"), '6'], [parseInt("2896056"), '5'], [parseInt("2642425"), '4'], [parseInt("2598097"), '3'], [parseInt("2512089"), '2'], [parseInt("2419735"), '1']]; // TODO -- add tooltip stuff for initial load too
  // load globl data
...
function initSVG() {
  svg = d3.select("body").append("svg").attr("width", w + margin.left + margin.right).attr("height", h + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); // Scale the range of the data in the domains

  x.domain([0, d3.max(barDataset, function (d) {
    return d[0];
  })]);
  y.domain(d3.range(1, barDataset.length + 1)); //Select…
...
function updateSVG(fullSongNames, barDataset, artistNames, songNames) {
  ...
  svg.append("g").attr("transform", "translate(0," + h + ")").call(d3.axisBottom(x)); // add the y Axis

  svg.append("g").call(d3.axisLeft(y).tickSize(0).tickFormat(""));
  svg.selectAll("title").remove();
  svg.selectAll("rect").append("title").text(function (d) {
    var idx = 10 - parseInt(d[1]); // NOTE: date is slider date (not dropdown date)

    return "\"" + fullSongNames[idx] + "\" by " + artistNames[idx] + ": " + d[0] + " streams on " + window[sliderDate];
  });
  var labels = svg.selectAll("text.value").data(barDataset).enter().append("text").text(function (d) {
    return songNames[parseInt(d[1]) - 1];
  }).attr("text-anchor", "end").attr("y", function (d, i) {
    return (9 - i) * (h / barDataset.length) + 27;
  }).attr("x", function (d) {
...
dateSelect = dateArr[0]; // update graph based on country dropdown
...
var dateDropdown = d3.select("#vis-container-date").insert("select", "svg").on("change", function () {
...
dateDropdown.selectAll("option").data(dateArr).enter().append("option").attr("value", function (d) {
  return d;
}).text(function (d) {
  return d;
}); // SLIDER STUFF !!
...
(function () {
  ...
  function playSlider() {} // Code inspired/provided by https://github.com/johnwalley/d3-simple-slider v1.5.4 Copyright 2019 John Walley
  ...
  function initSlider() {
    ...
    */
    ...
    gTime = d3.select('div#slider').append('svg').attr('width', 1350 - margin.left - margin.right).attr('height', 132 - margin.top - margin.bottom).append('g').attr('transform', 'translate(30,30)');
    gTime.call(sliderTime);
    gTime.selectAll("text").attr("dx", "-10px").attr("dy", "-16px"); //initializes date shown on screen
